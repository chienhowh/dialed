...
import { InvalidGuidedBrewPlanError } from "./presentation";
...
describe("Guided Brew Plan snapshot", () => {
  ...
  it("rejects a Plan without a usable step snapshot", () => {
    const brewPlan = {
      coffee: { productName: "Broken Plan" },
      coffeeDose: 15,
      grindLevel: "medium-fine",
      id: "plan-id",
      primaryTasteGoal: "sweet",
      ...
      waterAmount: 240,
      waterTemperature: 92,
    } as unknown as BrewPlan;

    expect(() => createGuidedBrewPlanSnapshot(brewPlan)).toThrow(InvalidGuidedBrewPlanError);
  });
});
